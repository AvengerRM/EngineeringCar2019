    ...
    else
    {
		...
		if(trigger_motor[1].press_l == 1 || switch_is_down(shoot_rc->rc.s[Shoot_RC_Channel]))
		{
			...
			if (rad_format(trigger_motor[1].set_angle - trigger_motor[1].angle) > 0.05f)
			{
				...
				//堵转判断
				if (trigger_motor[1].run_time - trigger_motor[1].cmd_time > 400 && trigger_motor[1].run_time - trigger_motor[1].cmd_time < 400 + 700 && fabs(trigger_motor[1].speed) < (0.5f))
				{
					trigger_motor[1].speed_set = - 8.0;
				}
				else if (trigger_motor[1].run_time - trigger_motor[1].cmd_time > 400 + 700)
				{
					trigger_motor[1].cmd_time = xTaskGetTickCount();
				}
			}
