...
//声明电机变量
static motor_measure_t motor_tongs[3], motor_chassis[4],motor_small_whell[2];
...
//返回底盘电机变量地址，通过指针方式获取原始数据
const motor_measure_t *get_Small_Whell_Motor_Measure_Point(uint8_t i)
{
    return &motor_small_whell[(i & 0x03)];
}

//返回底盘电机变量地址，通过指针方式获取原始数据
const motor_measure_t *get_Tongs_Motor_Measure_Point(uint8_t i)
{
    return &motor_tongs[(i & 0x03)];
}
 ...
 void CAN_hook(CAN_TypeDef* CANx,CanRxMsg *rx_message) 
{
    ...
    switch (rx_message->StdId)
    {
		...
		{
			//处理电机数据宏函数
			get_motor_measure(&motor_tongs[0], rx_message);
			//记录时间
		...
		{
			//处理电机数据宏函数
			get_motor_measure(&motor_tongs[1], rx_message);
		...
		{
			//处理电机数据宏函数
			get_motor_measure(&motor_tongs[2], rx_message);
			//记录时间
			break;
		}
		...
		case CAN_3508_M4_ID:
		{
			...
			{
				if(rx_message->StdId == CAN_FRIC_R_MOTOR_ID)
				 {
					//处理电机数据宏函数
					get_motor_measure(&motor_small_whell[0], rx_message);
					//记录时间
				 ...
				 {
					//处理电机数据宏函数
					get_motor_measure(&motor_small_whell[1], rx_message);
					//记录时间
					DetectHook(TriggerMotorTOE);
					break;					    
				 }
